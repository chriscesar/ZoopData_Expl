...
# taxon data - ID'd by PRNs
dfl %>% 
  dplyr::select(.,c(Pot.Number,PRN,DisplayName,Abund_m3)) -> dftax_m3_l
dftax_m3_l %>% 
  group_by(Pot.Number,PRN,DisplayName) %>% 
  summarise(.,Abund_m3 = sum(Abund_m3)) %>% 
  pivot_wider(.,
              names_from = DisplayName,
              values_from = Abund_m3,
              values_fill = 0) -> dftax_m3_w

# extract meta data
dfl %>% #names()
  dplyr::select(.,c(Pot.Number:Sample.comments,CEA.Notes:WB_lb)) -> dftax_meta_l

# dftax_meta_l %>% distinct()
...
df_carb <- readxl::read_xlsx(paste0(datfol,"Lifeforms/ZOOPLANKTON carbon mass data_v7Oct 2024_AMENDED_USE.xlsx"),
                             sheet = "FINAL SHEET TO USE") %>% #names()
  dplyr::select(.,c(2:6)) %>% 
...
### by WB ####
dfl %>%
  dplyr::select(., c(Region,PRN,WB,WB_lb,sample.date,yday,month,`Net.volume.sampled.(m3)`,LF02, Abund_m3)) %>%
  ...
  filter(.,str_starts(LF02,"Cop_")) %>%
  group_by(across(!c(Abund_m3))) %>% 
  dplyr::summarise(.,Abund_m3=sum(Abund_m3), .groups = "drop") %>% 
  ...
  labs(x="Day of year",
       ...
       # caption="Relative abundance of a given size class is the value recorded in a
       # sample divided by the maximum recorded value for that size class across all samples"
       )+
...
dfl %>%
  ...
  dplyr::select(., c(Region,PRN,WB,WB_lb,sample.date,month,DJF,yday,LF02, Abund_m3)) %>%
  # mutate(Cop_Sm=(`Net.volume.sampled.(m3)`*Cop_Sm)/max(`Net.volume.sampled.(m3)`*Cop_Sm),
  #        Cop_Ambi=(`Net.volume.sampled.(m3)`*Cop_Ambi)/max(`Net.volume.sampled.(m3)`*Cop_Ambi),
  #        Cop_Lg=(`Net.volume.sampled.(m3)`*Cop_Lg)/max(`Net.volume.sampled.(m3)`*Cop_Lg),
  #        Cop_NYA=(`Net.volume.sampled.(m3)`*Cop_NYA)/max(`Net.volume.sampled.(m3)`*Cop_NYA)) %>% 
...
dfl %>% 
  dplyr::select(.,WB_lb, sample.date, LF02, Abund_m3) %>% 
  group_by(across(!c(Abund_m3))) %>% 
  summarise(., Abund_m3 = sum(Abund_m3), .groups = "drop") %>% 
  filter(., str_starts(LF02, "Cop_") | str_starts(LF02, "Fish")) %>% 
  pivot_wider(.,names_from = LF02, values_from = Abund_m3,values_fill = 0) %>% 
